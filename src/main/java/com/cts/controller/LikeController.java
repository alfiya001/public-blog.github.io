...
//	@PreAuthorize("hasAuthority('USER') or hasAuthority('ADMIN') or hasAuthority('AUTHOR')")
    ...
    public ResponseEntity<?> likePost(@PathVariable (value = "postId") Long postId,
		...
		Optional<LikePost> optionalLike = likeRepositoy.findByPostIdAndUserId(postId, userId);
		if(optionalLike.isPresent())
			return ResponseEntity.internalServerError().body("Already_Liked");
		LikePost likeData = new LikePost();
		Post post = postRepository.findById(postId).get();
		likeData.setPost(post);
		AppUser user = userRepository.findById(userId).get();
		likeData.setUser(user);
		return ResponseEntity.ok(likeRepositoy.save(likeData));
...
//        }).orElseThrow(() -> new ResourceNotFoundException("PostId " + postId + " not found"));
	...
	// This function is used to get Like by Postid by alfiya Khan - 25/4/2022
	@PreAuthorize("hasAuthority('USER') or hasAuthority('ADMIN') or hasAuthority('AUTHOR')")
	@RequestMapping(value = "/likebyuser/{userId}", method = RequestMethod.GET)
	@ResponseBody
	public ResponseEntity<?> getLikeByUser(@PathVariable("userId") long userId) {
		try {
			List<LikePost> likes = likeRepositoy.findByUserIdOrderByPostId(userId);
			log.info("getLikeByUser: "+likes);
